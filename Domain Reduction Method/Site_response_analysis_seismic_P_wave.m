function [u_history,udot_history,uddot_history] = Site_response_analysis_seismic_P_wave
%--------------------------------------------------------------------------
...
Vp_ER = Vs_ER*sqrt((2-2*nu)/(1-2*nu));
damper_c_ER = rho_ER * Vp_ER;
...
for e = 1:nelems
    ...
    for i = 1:NEN
        if NodeXYCoordinate(ElementConnectivity(e,i+1),3) == L_bottom
            jC = jC+1;
            indexC(jC) = NodeXYCoordinate(ElementConnectivity(e,i+1),1)+nnodes;
            CKL_local_u2u2(i,i) = CKL_local_u2u2(i,i) + damper_c_ER*El_size/2; % mid points
        end
    end
    %--------------------------------------------------------------  
    ...
    CKL(ElementDof2GlDofu2,ElementDof2GlDofu2) = ...
        CKL(ElementDof2GlDofu2,ElementDof2GlDofu2) + CKL_local_u2u2;
    %----------------------------------------------------------------------
    % K matrix
    %----------------------------------------------------------------------
    % Ku1u1
...
% whatch out: here we only have constraints along the horizontal direction!!!
EssentialBCX = [(1:size(NodeXYCoordinate,1))',zeros(size(NodeXYCoordinate,1),1)];
KnownDof = [EssentialBCX(:,1)];
KnownST  = [EssentialBCX(:,2)];
